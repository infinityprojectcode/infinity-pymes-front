...
export default function ListCustomers() {
    ...
    const handleAddCustomer = () => {
        // setListCustomer((prev) => [...prev, { id: crypto.randomUUID, name: nameCustomer, category: categoryCustomer, price: priceCustomer, stock: stockCustomer }])
    }

    const handleSaveCustomer = () => {
        const customer = listCustomer.find(customer => customer.id === idCustomer)
    ...
    return (
        <div>
            {
                !showInfoCustomer && (
                    <button className='bg-blue-500 rounded-lg p-2 text-white' onClick={() => setShowAddCustomer(!showAddCustomer)}>Añadir nuevo</button>
                )
            }
            {
                !showAddCustomer && !showInfoCustomer ? (
                                ...
                                <li key={index} className='bg-200'>
                                    <div className='flex flex-col items-start justify-between gap-4'>
                                        <div className='flex gap-2'>
                                            <span className='text-100'>{item.first_name} {item.last_name}</span>
                                        </div>
                                        {item.status_count ? (
                                            <div className='flex items-start'>
                                                <CurrencyDolarIcon className={`${item.status_count ? 'text-500' : 'text-600'}`} /> <span className={`${item.status_count ? 'text-500' : 'text-600'}`} >{item.total_product_price}</span>
                                            </div>
                                        ) : (
                                            <div className='flex'>
                                                <CurrencyDolarIcon className={`${item.status_count ? 'text-500' : 'text-600'}`} /> <span className={`${item.status_count ? 'text-500' : 'text-600'}`} >{item.total_product_price}</span>
                                        ...
                                        <div className='w-full'>
                                            <div className='flex justify-between'>
                                                <button className='text-400' onClick={() => handleShowCustomer(item.id)} >Ver</button>
                                            </div>
                                        </div>
                                        {/* <div className='flex gap-2'>
                                            <button onClick={() => handleEditCustomer(item.id)} className='bg-blue-500 rounded-lg p-2 text-white'><svg xmlns="http://www.w3.org/2000/svg" className="icon icon-tabler icon-tabler-pencil" width="24" height="24" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" fill="none" strokeLinecap="round" strokeLinejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none" /><path d="M4 20h4l10.5 -10.5a2.828 2.828 0 1 0 -4 -4l-10.5 10.5v4" /><path d="M13.5 6.5l4 4" /></svg></button>
                                            <button onClick={() => handleDeletCustomer(item.id)} className='bg-red-500 rounded-lg p-2 text-white'><svg xmlns="http://www.w3.org/2000/svg" className="icon icon-tabler icon-tabler-trash" width="24" height="24" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" fill="none" strokeLinecap="round" strokeLinejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none" /><path d="M4 7l16 0" /><path d="M10 11l0 6" /><path d="M14 11l0 6" /><path d="M5 7l1 12a2 2 0 0 0 2 2h8a2 2 0 0 0 2 -2l1 -12" /><path d="M9 7v-3a1 1 0 0 1 1 -1h4a1 1 0 0 1 1 1v3" /></svg></button>
                                        </div> */}
                                    </div>
                                </li>
                            ))}
                    ...
                    <div className='flex flex-col pt-5'>
                        <div className='flex flex-row'>
                            <div className='w-2/5 grid'>
                                <div>
                                    <label className='text-100'>Ingresa los datos del cliente</label>
                                </div>
                                <div className='pt-2'>
                                    <label className='text-100'>Nombre:</label>
                                    <input className='w-full p-2 rounded-lg outline-none' type="text"
                                        value={nameCustomer}
                                        placeholder='Nombre'
                                ...
                                <div className='pt-2'>
                                    ...
                                    <input className='w-full p-2 rounded-lg outline-none' type="email"
                                        value={emailCustomer}
                                        placeholder='Email'
                                ...
                                <div className='pt-2 flex gap-8'>
                                    {/* <div>
                                        ...
                                        <input className='w-full p-2 rounded-lg outline-none' type="number"
                                            placeholder='$20.000'
                                            value={priceCustomer}
                        ...
                        </div>
                        {
                            idCustomer !== "" && nameCustomer !== '' ? (
                                <button className='bg-blue-500 rounded-lg p-2 text-white mt-4' onClick={() => handleSaveCustomer()}>Editar</button>
                            ) :
                                (
                                    <button className='bg-blue-500 rounded-lg p-2 text-white mt-4' onClick={() => handleAddCustomer()}>Añadir</button>
                                )
                        }
                    </div>
                ) : showInfoCustomer ? (
                    <div>
                        <div className='grid'>
                            ...
                            <button className='bg-blue-500 rounded-lg p-2 text-white mt-4' onClick={() => setShowInfoCustomer(!showInfoCustomer)}>Cerrar</button>
                        </div>
                    </div>
                ) : (
                    null
